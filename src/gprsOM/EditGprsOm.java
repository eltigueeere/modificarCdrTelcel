...
public class EditGprsOm {
	...
	mobil.MobilClass renglonesParametros = new mobil.MobilClass(); archivoInt.ArchivoInt renglonesMatirz = new archivoInt.ArchivoInt();
	
	public String[][] gprsOmEdit() throws FileNotFoundException, IOException {
		...
		if(archivoInt.existeArchivoEdit() == true) {
			...
			for(int i=0; i<parametros.length; i++) {
				...
				for(int j=0; j<divicion.divicionCdr(); j++) {
					...
					consumoCdrModificado[i][j] = 
							consumoCdr[consumoContador-1].substring(0,151) + fecha1[j] + consumoCdr[consumoContador-1].substring(165,254) +
							parametrosSplit[1] + consumoCdr[consumoContador-1].substring(269,275) + parametrosSplit[0] + 
							consumoCdr[consumoContador-1].substring(287,468) ;
			...
			log.logInfo("Error en el archivo a editar");
		}
